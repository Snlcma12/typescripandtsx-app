class Graph {
    ...
    PI2: number;
    WIN: { LEFT: number; BOTTOM: number; WIDTH: number; HEIGHT: number; };
    ...
    line(x1: number, y1: number, x2: number, y2: number, color = 'black', width = 0.5, isDash = false): void {
      this.ctxV.beginPath();
    ...
    text(text: string, x: number, y: number, color?: string, size?: string, font?: string): void {
      this.ctxV.beginPath();
      font = font || 'Arial';
      size = size || "12";
    ...
    point(x: number, y: number, color?: string, size: number = 1): void {
      this.ctxV.beginPath();
